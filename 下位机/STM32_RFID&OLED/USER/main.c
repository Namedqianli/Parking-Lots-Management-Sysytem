...
int main(void)
{
	...
	Lcd_Clear(GRAY0);
	Gui_DrawFont_GBK16(0,30,BLUE,GRAY0,"Entry Time");
	Gui_DrawFont_GBK16(0,70,BLUE,GRAY0, "Left TIme");
	delay_ms(1000);
	GPIO_ini();
  InitializeSystem( );
  uart_init(115200);
...
//	  for(j=0;j<2000;j++);
  ...
  //OLED_ShowString1(0,0," RFID EPC SYS");
	...
	while(1)
	{
          ...
          status = PcdAuthState(KEYA, (snr*4+3), DefaultKey, SelectedSnr);// 校验1扇区密码，密码位于每一扇区第3块
				...
				//			status = PcdWrite((snr*4+0), buf);  // 写卡，将buf[0]-buf[16]写入1扇区0块
							if(!status)
							{
								...
								if(mode == REGIST)
								{
									printf("*0%s#", SelectedSnr);
									delay_10ms(1);
									mode = MONITIOR;
								}
								else
								{
									printf("*1%s#",SelectedSnr);
									delay_10ms(1);
								}
								WaitCardOff();
							}
		...
		delay_10ms(1);
		LED_OFF; 
		if(USART_RX_STA&0x8000)				//检查是否有发送信息过来
		{
			USART_RX_STA=0;							//把接受标志位继续打开
			//对接受到的信息进行处理
			if(USART_RX_BUF[0] == '#')
			{
				printf("!");	//返回连接确认
			...
			else if(USART_RX_BUF[0] == '1')		//进入时间
			{
				p = &USART_RX_BUF[1];
				Gui_DrawFont_GBK16(0,50,BLUE,GRAY0,p);
				
				
			}
			else if(USART_RX_BUF[0] == '2')		//离开时间
			{
				p = &USART_RX_BUF[1];
				Gui_DrawFont_GBK16(0,100,BLUE,GRAY0,p);	
			}
			else if(USART_RX_BUF[0] == '3')		//余额
			{
				p = &USART_RX_BUF[1];
				Gui_DrawFont_GBK16(0,130,BLUE,GRAY0, p);
			}
			p = NULL;
